...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                var userFromDb = _context.Set<AspNetUser>().Where(n => n.UserName == model.Username).FirstOrDefault();
                
                if (userFromDb != null && IsPasswordValid(model.Password , userFromDb.PasswordHash))
                {
                    

                    string subId = GetSubId(userFromDb.Id);

                    await _events.RaiseAsync(new UserLoginSuccessEvent(userFromDb.UserName, subId.ToString(), userFromDb.UserName, clientId: context?.Client.ClientId));
                    ...
                    var isuser = new IdentityServerUser(subId.ToString())
                    {
                        DisplayName = userFromDb.UserName,
                        AdditionalClaims = GetClaims(userFromDb)
                    };
        ...
        public bool IsPasswordValid(string password , string passwordFromDB)
        {
            string chackedPassword = password.Sha256();
            return chackedPassword == passwordFromDB;
        }


        public string GetSubId(string s)
        {
            return GetNumbers(s);
        }
